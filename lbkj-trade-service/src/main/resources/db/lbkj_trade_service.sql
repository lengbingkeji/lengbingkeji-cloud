CREATE database if NOT EXISTS `lbkj_trade_service` default character set utf8mb4 collate utf8mb4_general_ci;
USE `lbkj_trade_service`;
...
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Dynamic COMMENT ='订单表';

-- 正在导出表  order 的数据：~6 rows (大约)
DELETE FROM `order`;
...
-- 正在导出表 order_detail 的数据：~10 rows (大约)
...
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Dynamic COMMENT='order_logistics表';

-- 正在导出表  order_logistics 的数据：~1 rows (大约)
